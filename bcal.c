...
bool chs2lba(char *chs, maxuint_t *lba)
{
	...
	*lba += param[2] - 1; /* S - 1 */
	...
	fprintf(stdout, "\tC %lu H %lu S %lu MAX_HEAD %lu MAX_SECTOR %lu\n",
...
bool lba2chs(char *lba, t_chs *p_chs)
{
	...
	fprintf(stdout, "MAX_HEAD %s ", getstr_u128(param[1], uint_buf));
